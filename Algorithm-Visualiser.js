class algorithmVisualiser{
  configVariables(){ // Bodge for ES6 not supporting Public Field Declarations
    self.dataSet = {
      ...
      lessthan:    (x,y) => {  
      		self.dataSet.compIndicator = [x, y]; 
      		self.dataSet.compCount++; 

      		return self.dataSet.data[x] < self.dataSet.data[y]
      	},
  ...
  draw(){
    if(self.options.fps != false){
    	self.options.fps.begin();
    }
    let ctx = self.ctx;
